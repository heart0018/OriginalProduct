#!/usr/bin/env python3
"""
関西地方多カテゴリスポット自動取得スクリプト
Google Places APIを使用して関西の温泉・公園・サウナ・カフェデータを取得し、MySQLに保存する
"""
...
class KansaiDataCollector:
    ...
    def search_places(self, query: str, location: str = "", radius: int = 100000) -> List[Dict]:
        """Google Places APIでテキスト検索（関西全域対応）"""
        # 位置情報がない場合は関西地方の中心付近を使用
        if not location:
            params = {
                'query': query,
        ...
        try:
            ...
            # 関西地方の住所を持つ結果のみフィルタリング
            kansai_results = []
            for result in results:
                address = result.get('formatted_address', '')
                if any(prefecture in address for prefecture in self.kansai_prefectures):
                    kansai_results.append(result)

            print(f"📍 関西地方内: {len(kansai_results)}件の候補を発見")
            return kansai_results
    ...
    def _get_existing_counts(self, category: str):
        """指定カテゴリの既存総数と都府県別カウントを取得"""
        connection = self.connect_database()
        total = 0
        prefect_counts = {p: 0 for p in self.kansai_prefectures}
        if not connection:
            return total, prefect_counts
        try:
            cur = connection.cursor()
            # 総数
            cur.execute("SELECT COUNT(*) FROM cards WHERE genre=%s AND region='関西'", (category,))
            total = cur.fetchone()[0]
            # 住所を取得して都府県判定
            cur.execute("SELECT address FROM cards WHERE genre=%s AND region='関西'", (category,))
    ...
    def collect_data(self, category: Optional[str] = None):
        """均等配分アルゴリズムでのデータ収集（都府県ごとのクォータ厳守）
        category を指定した場合、そのカテゴリのみ不足分を追加収集(トップアップ)する"""
        print("🚀 関西全域多カテゴリデータ収集開始 (均等/トップアップモード)")
        ...
        def build_prefecture_quotas(total: int) -> Dict[str, int]:
            base = total // len(self.kansai_prefectures)
            rem = total % len(self.kansai_prefectures)
            quotas = {}
        ...
        for cat in categories:
            ...
            # 既存分を考慮した都府県別不足計算 (理想=full_targetを均等割当)
            ideal_full = build_prefecture_quotas(full_target)
            deficits = {p: max(0, ideal_full[p] - existing_pref_counts.get(p, 0)) for p in self.kansai_prefectures}
            ...
            if total_deficit == 0:
                ...
                # 不足が多い都府県を優先しつつ remaining_target を配分
                quotas = {p: 0 for p in self.kansai_prefectures}
                ...
                if need > 0:
                    ...
                    for p, v in tmp.items():
                        quotas[p] += v
            # 0割当都府県は収集ループでスキップされる
            print(f"🧮 都府県別追加クォータ: {quotas}")
            ...
            counts = {p: 0 for p in self.kansai_prefectures}
            exhausted = {p: quotas.get(p, 0) == 0 for p in self.kansai_prefectures}
            zero_gain_streak = {p: 0 for p in self.kansai_prefectures}
            ...
            while sum(counts.values()) < target and not all(exhausted.values()):
                ...
                for pref in self.kansai_prefectures:
                    ...
                    if not prefecture_query_map[pref]:
                        ...
                        if cat == 'relax_onsen' and pref in ['滋賀', '和歌山', '奈良']:
            ...
            if cat == 'active_sauna' and deficit > 0:
                ...
                # 不足都府県のみ
                deficit_prefs = [p for p in self.kansai_prefectures if quotas.get(p,0) > 0 and counts[p] < quotas[p] + REALLOC_ALLOW_DIFF]
                if not deficit_prefs:
                    deficit_prefs = self.kansai_prefectures  # 念のため
                rounds2 = 0
